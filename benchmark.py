        ...
        if platform.system() == "Windows":
            ...
            for line in lines:
                ...
                if "NVIDIA" in upper_line or "RTX" in upper_line or "GTX" in upper_line:
                    return line
                if "RADEON RX" in upper_line or ("RADEON" in upper_line and "GRAPHICS" not in upper_line):
                    return line
            return lines[0] 
            
        elif platform.system() == "Linux":
            output = subprocess.check_output("lspci -vnn | grep VGA", shell=True, text=True)
            return output.strip()
    except Exception:
        pass
    return "Unknown GPU"
...
class Benchmark:
    def __init__(self, mode):
        ...
        if self.mode in ["GL_RAW", "GL_SHADER"]:
            ...
            self.display = pygame.display.set_mode(config.window_size, display_flags, vsync=False)
            
            if self.mode == "GL_SHADER":
                self.shader_manager = ShaderManager(config.INTERNAL_RESOLUTION, config.window_size)
                ...
                self.raw_gl_surface = moderngl_utils.GLSurface(config.INTERNAL_RESOLUTION, config.window_size, self.texture_handler)
                self.render_surface = self.raw_gl_surface.surface
        else:
            self.display = pygame.display.set_mode(config.window_size, display_flags, vsync=False)
            self.shader_manager = None
            self.render_surface = pygame.Surface(config.INTERNAL_RESOLUTION, pygame.SRCALPHA)
    ...
    def run_phase(self, phase):
        ...
        while time.time() - start_time < duration:
            ...
            if self.mode == "GL_SHADER":
                ...
                scaled = pygame.transform.scale(self.render_surface, self.display.get_size())
    ...
    def save_report(self, final_score):
        ...
        for res in self.results:
            report.append(f" - {res['phase']:<20}: {res['particles']} particles -> {res['avg_fps']:.1f} FPS")
    ...
    def run(self):
        ...
        while True:
            ...
            header = self.font.render(f"BENCHMARK COMPLETE [{self.mode}] - SAVED", True, (0, 255, 150))
