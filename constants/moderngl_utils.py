...
def create_data_for_quad_buffer(size:tuple, position:tuple, screen_size:tuple):
    def convert_coord(c:int, screen_half_size:int):
    ...
    return array.array("f", [
        convert_coord(position[0] - half_size[0], screen_half_size[0]), convert_coord(position[1] - half_size[1], screen_half_size[1]), 0.0, 0.0, #top left
        convert_coord(position[0] + half_size[0], screen_half_size[0]), convert_coord(position[1] - half_size[1], screen_half_size[1]), 1.0, 0.0, #top right
        convert_coord(position[0] - half_size[0], screen_half_size[0]), convert_coord(position[1] + half_size[1], screen_half_size[1]), 0.0, 1.0, #bottom left
        convert_coord(position[0] + half_size[0], screen_half_size[0]), convert_coord(position[1] + half_size[1], screen_half_size[1]), 1.0, 1.0 #bottom right  
    ])
...
class Shader:
    def __init__(self, vertex_shader:str, fragment_shader:str):
        self.gl_context = moderngl.get_context()
        self.program = self.gl_context.program(vertex_shader, fragment_shader)
    ...
    def draw(self, texture_hash:int, size:tuple, screen_size:tuple, position:tuple|None = None, **kwargs):
        ...
        self.program["surface"] = texture_hash
        for key, arg in kwargs.items():
            self.program[key] = arg
...
class GLSurface:
    ...
    def draw(self, **kwargs):
        hash = self.texture_handler.write_to_memory(from_surface_to_texture(self.gl_context, self.surface))
        self.shader.draw(hash, self.size, self.screen_size, self.position, **kwargs)
